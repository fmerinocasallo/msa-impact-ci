...
def read_arguments () :
    ...
    """
    parser = argparse.ArgumentParser(description=('Remove gene sections for '
                                                  'each given sequence')
                                    )
...
def get_gene_id ( feature, record_id ) :
    ...
    Arguments :
        ...
        record_id ( string )
            record identifier from a SeqRecord object
...
def remove_gene ( input_filename, gene_id ) :
    """
    [ Generator function ]

    Try to remove 'gene_id' sections for each sequence stored in 'input_filename'.
    ...
    """
    def gene_search_and_removal ( record, gene_id ) :
        """
        ...
        Returns :
            SeqRecord
                The very same 'record' without 'gene_id' sections.
            ...
            for feature in record.features[1:] :
                ...
                if gene in ['tRNA-Ser', 'tRNA-Leu'] :
                    if (gene + '1') in record_genes :
                        gene +=  '2'
                    else : # (gene + '1') not in record_genes
                        gene += '1'
                ...
                if gene == gene_id :
                    ...
                    upper_bound = -1
                    # Remove 'gene_id' sections from record
                    if feature.location :
                        ...
                        # will have to remove them using rCRS scheme.
                            ...
                            for location in sorted_locations :
                                if upper_bound == -1 :
                                    if gene_id == 'D-loop' :
                                        ...
                                        if location.end < 500 :
                                            ...
                                            for f in record.features[1:] :
                                                if f.type != 'tRNA' :
                                                    ...
                                                    tRNA_found = True
                                                    tRNA_start = f.location.start
                                                    tRNA_id = \
                                                        f.qualifiers['product'][0]
                                                    if (location.end \
                                                            == tRNA_start) :
                                                        print(('{:s},,WARNING: {:s} - It ends at '
                                                               '{:d} but {:s} starts '
                                                               'at {:d},,,,'
                                                               ''.format(gene_id,
                                                                    record.id,
                                        ...
                                        new_record.seq = \
                                                    record.seq[0:location.start]

                                    upper_bound = location.end
                                else : # upper_bound != -1
                                    new_record.seq += \
                                        record.seq[upper_bound:location.start]
                                    ...
                                    if gene_id == 'D-loop' :
                                        ...
                                        if location.end < len(record.seq) - 1:
                                            # print(('{:s},,{:s},,,,'
                                            #        ''.format(gene_id, record.id)))
                                            print(('{:s},,{:s} - It ends at {:d} '
                                                   'instead of {:d},,,,'
                                                   ''.format(gene_id, record.id,
                                                             location.end,
                                                             len(record.seq) - 1)))
                                            invalid_info = True
                                            # upper_bound = len(record.seq) - 1
                                        else : # location.end == len(record.seq) - 1
                                            upper_bound = len(record.seq)

                                    else : # gene_id != 'D-loop'
                                        upper_bound = location.end
                                        #FIXME We have to include the last fragment of the sequence [upper_bound:]
                ...
                if gene_id == 'D-loop' :
                    ...
                    for feature in record.features[1:] :
                        if feature.type != 'tRNA' :
                            continue
                        else : # feature.type == 'tRNA'
                            tRNA_found = True
                            tRNA_start = feature.location.start
                            print(record.id)
                            if ( 'product' in feature.qualifiers ) :
                                tRNA_id = feature.qualifiers['product'][0]
                            elif ( 'gene' in feature.qualifiers ) :
                                tRNA_id = feature.qualifiers['gene'][0]
                            else :
                                print('{:s},,,,{:s},,'.format(gene_id, record.id))
                                break

                            if (tRNA_start == 0) :
                                print(('{:s},,,,WARNING: {:s} - {:s} starts '
                                       'at {:d},,'
                                       ''.format(gene_id, record.id,
                                                 tRNA_id, tRNA_start)))
                            else : # tRNA_start != 0
    ...
    print(('gene id, D-loop w/o 2 sections,D-loop wrong boundaries,'
           'no locations info, no gene id features, no features, unable to '
           'extract'))

    for record in Bio.SeqIO.parse(input_filename, 'gb') :
        new_record = gene_search_and_removal(record, gene_id)
        if new_record :
            # 'gene_id' sections have been successfully removed
            yield new_record, None
        else :
            # Unable to remove 'gene_id' sections because 'record' doesn't
...
def main () :
    ...
    filename_processables = (filename + '_without_' + args.gene_id + '_' +
                             'processables.fasta')
    filename_unprocessables = (filename + '_without_' + args.gene_id + '_' +
    ...
    with open(filename_processables, 'w') as handle_processables, \
        ...
        sequences = remove_gene(args.input_filename, args.gene_id)
        ...
        print(('We\'ve removed {:s} gene fragments from {:d} sequences '
               'and store them in {:s}'
               ''.format(args.gene_id, num_processables,
                         filename_processables)))

        print(('We\'ve found {:d} sequences where we\'ve been unable to remove '
